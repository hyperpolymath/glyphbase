...
This directory contains different view components for rendering table
data.
...
[width="100%",cols="19%,42%,39%",options="header",]
|===
...
|`+onUpdateRow+` |`+(string, string, cellValue) => unit+` |Callback when
a card is moved
|===
...
|`+onEventClick+` |`+option<row => unit>+` |Optional callback when event
is clicked
|===
...
Card-based gallery view for displaying visual content with images and
metadata.
...
|`+coverFieldId+` |`+option<string>+` |Optional ID of Attachment or URL
...
|`+onCardClick+` |`+option<row => unit>+` |Optional callback when card
is clicked
...
Public-facing form view for data entry with validation and success
handling.
...
|`+config+` |`+formConfig+` |Form configuration (title, description,
messages)
...
|`+showRequiredIndicator+` |`+bool+` |Show asterisk for required fields
(default: true)
|===
