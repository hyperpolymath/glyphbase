...
The `+export+` functions in Lithoglyph bridge.zig are C-exported but not
Zig-public.

*Solution*: Either: - Use C FFI to call them (via `+@extern+`) - Add
`+pub+` before `+export+` in Lithoglyph bridge.zig - Link against
compiled Lithoglyph library instead of importing source
...
Instead of importing Lithoglyph as a module, we can link against it as a
C library:
...
[arabic]
. Install Erlang dev headers
. Either fix Lithoglyph pub exports or use C FFI approach
. Test Lithoglyph compatibility with Zig 0.15.2
